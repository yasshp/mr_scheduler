...
# ─── MERGE CONTACT INFO (Phone & Segment) ──────────────────────────
# We try to load the Contacts sheet to get the phone number and segment
try:
    contacts_df = db.load_data("Contacts")
    if not contacts_df.empty:
        # Standardize column names to avoid key errors
        contacts_df.columns = contacts_df.columns.str.strip().str.lower()
        
        # Find the ID column
        id_col = next((c for c in ['contact_id', 'id', 'customer_id'] if c in contacts_df.columns), None)
        
        if id_col:
            # Merge to attach phone and segment to the daily schedule
            daily = daily.merge(
                contacts_df, 
                left_on='customer_id', 
                right_on=id_col, 
    ...
    pass # If Contacts sheet fails, we just show N/A

# ─── HELPER FUNCTIONS ──────────────────────────────────────────────
def get_col(row, candidates, default="N/A"):
    # Smarter get_col that checks lowercase keys too
    row_keys = [k.lower() for k in row.index]
    for col in candidates:
        if col.lower() in row_keys:
            actual_key = row.index[row_keys.index(col.lower())]
            val = row[actual_key]
    ...
    with st.expander("🗺️ View Daily Route Map", expanded=True):
        if 'latitude' in daily.columns and 'longitude' in daily.columns:
             ...
             if not valid_points.empty:
                ...
                # Added returned_objects=[] to prevent glitching
                st_folium(m, width=None, height=400, returned_objects=[])
             else:
                 st.caption("No valid GPS coordinates found for this route.")
        else:
...
with c_todo:
    ...
    for idx, row in pending.iterrows():
        ...
        # 🟢 NEW: Get Phone & Segment
        phone = get_col(row, ['phone', 'mobile', 'contact_number'], 'N/A')
        segment = get_col(row, ['segment', 'category'], 'General')
        ...
        with st.container(border=True):
            ...
            st.markdown(f"### {row['customer_name']}")
            if dr_name: st.markdown(f"**👨‍⚕️ {dr_name}**")
            
            # 🟢 NEW: Display Phone & Segment
            st.markdown(f"🏷️ **{segment}**")
            if phone != 'N/A':
